...
export const progressService = {
  ...
  getMyCourseProgress: async (courseId: string): Promise<StudentCourseProgressDetail> => {
    const { data } = await apiClient.get(`/progress/courses/${courseId}/my-progress`);
    return data.data;
  },

  setLessonState: async (lessonId: string, state: LessonProgressState) => {
    const { data } = await apiClient.patch(`/progress/lessons/${lessonId}/state`, { state });
    return data.data;
  },

  markLessonComplete: async (lessonId: string, isCompleted = true) => {
    const { data } = await apiClient.patch(`/progress/lessons/${lessonId}/complete`, { isCompleted });
    return data.data;
  },
};
