...
export function DirectChatPage() {
  ...
  const effectiveSelectedRoomId = selectedRoomId ?? directRooms[0]?.id ?? null;
  ...
  useEffect(() => {
    if (!token || !effectiveSelectedRoomId) {
      return;
    }
  ...
  const selectedRoom = directRooms.find((room) => room.id === effectiveSelectedRoomId) ?? null;
              ...
              <Space direction="vertical" size={16} style={{ width: '100%' }}>
                ...
                <div className="community-chat-composer">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (!effectiveSelectedRoomId || !draftMessage.trim()) {
                        return;
                      }
