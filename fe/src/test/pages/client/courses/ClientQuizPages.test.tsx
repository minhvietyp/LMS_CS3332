...
describe('Client quiz pages', () => {
  ...
  it('renders published course quizzes with attempt availability', async () => {
    ...
    expect(await screen.findByText('React Checkpoint', {}, { timeout: HEAVY_UI_TEST_TIMEOUT * 2 })).toBeInTheDocument();
    await waitFor(() => {
  ...
  it('starts and submits a quiz attempt, then navigates to the result page', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /A library/i }));
    ...
    await waitFor(
      () => {
        expect(screen.getAllByText('Score 100%').length).toBeGreaterThan(0);
      },
      { timeout: 10000 },
    );
  ...
  it('shows exhausted attempt guidance and keeps review access when no attempts remain', async () => {
    ...
    expect(await screen.findByText('No attempts remain for this quiz.')).toBeInTheDocument();
